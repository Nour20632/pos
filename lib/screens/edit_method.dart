...
class LabelPrintingScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: const Center(child: Text('Impression étiquettes à implémenter')),
    );
  }
}
...
class EditProductScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Text('Modification produit $productId à implémenter'),
      ),
    );
  }
}
...
class EditCustomerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Text('Modification client $customerId à implémenter'),
      ),
    );
  }
}
          ...
          body: Padding(
            ...
            child: ListView(
              children: [
                ...
                Text(
                  'Montant total: ${invoice.totalAmount.toStringAsFixed(2)} DA',
                ),
                const Divider(),
                const Text(
                  'Articles:',
                ...
                ...invoice.items.map(
                  (item) => ListTile(
                    title: Text(item.productName),
                    subtitle: Text(
                      'Qté: ${item.quantity} | Prix: ${item.unitPrice.toStringAsFixed(2)} DA',
                    ),
                    trailing: Text('${item.totalPrice.toStringAsFixed(2)} DA'),
                  ),
                ),
                const Divider(),
                Text('Statut: ${invoice.paymentStatus.name.toUpperCase()}'),
                if (invoice.remainingAmount > 0)
                  Text(
                    'Reste à payer: ${invoice.remainingAmount.toStringAsFixed(2)} DA',
